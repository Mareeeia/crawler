...
public class URLValidatorTest {
    ...
    @Test
    public void testValidatesNonFile() {
        var urlValidator = new URLValidator("sample.com");
        var variousDomainsList = List.of(
                "http://www.sample.com",
                "http://sample.com?=something/sample.jpeg",
                "http://sample.com?=something/sample.pdf",
                "http://sample.com?=something/sample.docx",
                "http://sample.com?=something/sample.mp3"
        );

        var resultList = variousDomainsList.stream()
                .filter(urlValidator::isRequiredUrl).toList();

        assertThat(resultList, containsInAnyOrder("http://www.sample.com"));
    }
}
